    ...
    for (y=0; y < height; y++) {
        for (x=0; x < width; x++) {
            if (((x % tile_width) == 0) ||
                ...
                pixel_draw_contrast(x, y, p_overlaybuf);
            }
        }
    }
    ...
    for (y=0; y < height; y+= tile_height) {
        for (x=0; x < width; x+= tile_width) {
            ...
            font_render_number(x + 2,
                               y + 2,
                               test++, // x + (y * (width/tile_width)),  // number to render -> tile index
                               p_overlaybuf);
        }
    }
...
//    for (x=0; x < 10; x++)
        // font_render_digit(x * tile_width + 2, 2, x, p_overlaybuf);
